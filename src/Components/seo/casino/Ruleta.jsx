...
export default function Ruleta(props) {
  ...
  const floor1 = {
    ...
    questions: {
      ...
      inner: [
        {
          h3: "Una Guía en los diferentes Tipos de Apuesta",
          text: [
        ...
        {
          h3: "¿Qué hace único jugar a la Ruleta en Codere?",
          text: [
    ...
    <div className="cas-seo">
      ...
      <Container style={{ color: "#fff" }}>
        ...
        <div id={floor1.id}>
          ...
          <div className={"isnvisible"} id={"anchor-5"}></div>
          <Accordion>
            {floor1.questions.inner.map((t, k) => (
              <Accordion.Item key={k} eventKey={k}>
                <Accordion.Header as={"h3"}>{t.h3}</Accordion.Header>
                <Accordion.Body>
                  {t.text.map((pi, k) => (
                    <p key={k}>{pi}</p>
                  ))}
