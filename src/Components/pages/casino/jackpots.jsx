...
export default function Jackpots(props)
{
	...
	// 			acceptedAnswer: {
	// 				"@type": "Answer",
	// 				text   : "El objetivo de este juego es sumar 21 puntos o no pasarse de esta cifra, pero siempre sobrepasando el
	// valor que tiene el crupier para ganar la apuesta.", }, }, { "@type"       : "Question", name          : "¿Cómo se cuentan las cartas
	// en el blackjack?", acceptedAnswer: { "@type": "Answer", text   : "Dale el valor de -1 a las cartas altas (del 10 al A), +1 a las
	// bajas (del 2 al 6) y 0 al resto. Empieza a contar tras el reparto dando el valor a cada carta que sale de la baraja.", }, }, {
	// "@type"       : "Question", name          : "¿Cómo ganar dinero en el juego 21?", acceptedAnswer: { "@type": "Answer", text   : "El
	// As es la carta más fuerte en el BlackJack porque vale 11 puntos, por lo que la mejor opción es dividir los ases si tienes más de uno
	// y así tener dos opciones para conseguir un 21 (BlackJack). Con los ochos pasa algo parecido, ya que juntos suman 16, que es la peor
	// mano posible en este juego.", }, }, { "@type"       : "Question", name          : "¿Cuántos tipos de blackjack online existen?",
	// acceptedAnswer: { "@type": "Answer", text   : "Hay dos tipos o estilos de blackjack, el europeo y el americano, que, aunque parezcan
	// iguales, tienen unas pequeñas diferencias: En el americano, el crupier se reparte dos cartas, una descubierta y otra cubierta, junto
	// con los otros jugadores. En el europeo solo se reparte una carta descubierta y dependiendo de lo que haya salido deben pasar otra ronda de apuestas antes de recibir el siguiente naipe.", }, }, { "@type"       : "Question", name          : "¿Qué significa blackjack?", acceptedAnswer: { "@type": "Answer", text   : "El juego consiste en sumar un valor lo más próximo a 21 pero sin pasarse. Esta jugada se conoce como Blackjack o 21 natural.", }, }, ], };
	...
	const reglas = {
		...
		p : {
			p1          : "Te preguntarás, ¿cuál es la mejor hora para jugar y cazar un jackpot millonario? Bueno, la respuesta a cualquier hora puede ser la mejor. Eso es lo más divertido de los slots, ya que cuando menos te lo esperes tú puedes salir ganando. Es cierto, también puedes hacer un plan para divertirte y poder ser uno de los ganadores, aunque nunca está de más cruzar los dedos.\n" +
				"\n",
			innerTitle  : "4 CONSEJOS PARA AGARRAR UN PREMIOS MILLONARIO EN LOS JACKPOT\n",
	...
	const floor1 = {
		...
		data : {
			...
			p2: ["¡Relájate! Sabemos que hay una amplia variedad de juegos, por eso hemos creado una lista con la mejor selección de slots, juegos que su trama y temática te llevará a divertirte y ganar.\n" +
			"\n",
	...
	return (
		<div className="cas-seo">
			...
			<Container style={{color: "#fff"}}>
				...
				<div>
					<h2 id={reglas.id} className="mt-4 mb-3">{reglas.h2}</h2>
					<p>{reglas.p.p1} </p>
					<h2 id={reglas.p.innerTitleId} className="mt-4 mb-3">{reglas.p.innerTitle}</h2>
					<ul>
